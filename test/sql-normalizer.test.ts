...
import { fingerprint, normalize } from "../src/sql-normalizer.js";
...
describe("normalize", () => {
  ...
  it("replaces string literals", () => {
    expect(normalize("SELECT * FROM users WHERE name = 'Alice'")).toBe("SELECT * FROM users WHERE name = ?");
  });

  it("replaces double-quoted strings", () => {
    expect(normalize('SELECT * FROM users WHERE name = "Bob"')).toBe("SELECT * FROM users WHERE name = ?");
  });
  ...
  it("replaces booleans", () => {
    expect(normalize("WHERE active = TRUE AND deleted = FALSE")).toBe("WHERE active = ? AND deleted = ?");
  });
